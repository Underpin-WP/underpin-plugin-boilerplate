...
abstract class Batch_Task {
	...
	public function run( $current_tally ) {
		...
		if ( is_wp_error( $valid ) ) {
			return plugin_name_replace_me()->logger()->log_wp_error( 'batch_error', $valid );
		}
		...
		for ( $i = 0; $i < $this->tasks_per_request; $i++ ) {
			...
			if ( is_wp_error( $status ) ) {
				plugin_name_replace_me()->logger()->log_wp_error( 'batch_error', $status );
	...
	public function render_callback() {
		if ( is_admin() && current_user_can( $this->capability ) ) {
			...
			plugin_name_replace_me()->scripts()->set_param( 'batch', $this->batch_id, $batch_params );
			plugin_name_replace_me()->scripts()->enqueue( 'batch' );
			plugin_name_replace_me()->styles()->enqueue( 'batch' );
